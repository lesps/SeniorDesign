...
%I think the bullets could use more - where do we get the content? Where do we actually figure out the AR stuff? 
\begin{itemize}
\item Build OCR (optical character recognition) engine, or become familiar with a library
\item Build basic backend for processing equations and keywords found in text (possibly including an NLP library)
\item Create a system that determines what kind of graphic to show based on input
\item Build a protocol that will allow us to render these graphics on the client
\end{itemize}
...
\begin{itemize}
\item How to read the input (via OCR) without additional markings on the paper
\item How to break down the input and determine what we could possibly show as visuals
\item How to determine which visual would be best to display
\item How to actually display the visual on the end client
\item How to save on battery life on the end client
\item How to reduce latency of the overall system 
\end{itemize}
